...
int main(int argc, char **argv)
{
    ...
    double time, time1, time2, shortestTime = 99999, firstTime = 1, pi;
    int repeat = 32, i;
...
#pragma omp parallel num_threads(number_of_threads)
            {
                ...
                struct timespec startCrit, finishCrit1, finishCrit2;
                clock_gettime(CLOCK_MONOTONIC, &startCrit);
                local_sum += calcSum(dx, intervals, number_of_threads);
                clock_gettime(CLOCK_MONOTONIC, &finishCrit1);
#pragma omp critical
                global_sum += local_sum;
                // end critical section

                clock_gettime(CLOCK_MONOTONIC, &finishCrit2);
#pragma omp critical
                time1 += timeSpend(finishCrit1, startCrit);
                time2 += timeSpend(finishCrit2, startCrit);
                //printf("timespent = %f ,critical time percent=%f\n", time2, ((time - time1) / time2) * 100);
            }
        ...
        measurements[number_of_threads - 1].timeCrit = ((time2-time1)/time2)*100;
    }
    for (i = 0; i < 16; i++)
    {
        printf("-------------threads %d ----------------\n", i + 1);
    ...
    for (i = 0; i < 16; i++)
    {
        ...
        printf("%.2f & %.2f & %.2f, critical time percent %f\n", measurements[i].time * 100, firstTime / measurements[i].time, (firstTime / measurements[i].time) / (i + 1),measurements[i].timeCrit);
    }
